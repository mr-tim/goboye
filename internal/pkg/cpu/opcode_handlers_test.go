...
func TestAdd8BitSignedToSPSaveInHL(t *testing.T) {
	doTestAdd8BitSignedToSpSaveInHL(t, 0x0000, 0xFF, 0xFFFF, false, false)
	doTestAdd8BitSignedToSpSaveInHL(t, 0x0001, 0x01, 0x0002, false, false)
	doTestAdd8BitSignedToSpSaveInHL(t, 0x0001, 0xFF, 0x0000, true, true)
	doTestAdd8BitSignedToSpSaveInHL(t, 0x00FF, 0x01, 0x0100, true, true)
	doTestAdd8BitSignedToSpSaveInHL(t, 0x0100, 0xFF, 0x00FF, false, false)
	doTestAdd8BitSignedToSpSaveInHL(t, 0x0FFF, 0x01, 0x1000, true, true)
	doTestAdd8BitSignedToSpSaveInHL(t, 0x1000, 0xFF, 0x0FFF, false, false)
...
func doTestAdd8BitSignedToSpSaveInHL(t *testing.T, spValue uint16, operand uint8, expected uint16,
	...
	t.Run(fmt.Sprintf("SP = %04X + %02X (expect = %04X, C:%v H:%v)", spValue, operand, expected, carry, halfCarry),
		func(t *testing.T) {
			p := setupHandlerTest([]byte{0xF8, operand})
			p.registers.sp = spValue
			p.DoNextInstruction()
			assert.Equal(t, expected, p.registers.hl)
			assert.Equal(t, false, p.GetFlagValue(FlagZ), "Incorrect Z Flag")
			assert.Equal(t, false, p.GetFlagValue(FlagN), "Incorrect N Flag")
			assert.Equal(t, halfCarry, p.GetFlagValue(FlagH), "Incorrect H Flag")
			assert.Equal(t, carry, p.GetFlagValue(FlagC), "Incorrect C Flag")
		})
}
